...
bool EV1527Protocol::verifyOne(uint16_t h, uint16_t l) {
    float ratio = (float)h / l;
    return ratio > GENERIC_RATIO_MIN && ratio < GENERIC_RATIO_MAX;
}

bool EV1527Protocol::verifyZero(uint16_t h, uint16_t l) {
    float ratio = (float)l / h;
    return ratio > GENERIC_RATIO_MIN && ratio < GENERIC_RATIO_MAX;
}

bool EV1527Protocol::verifySync(uint16_t sync, uint16_t data) {
...
bool EV1527Protocol::tryParse(const uint16_t* timings, size_t count, ProtocolData& out, size_t& out_timingsConsumed) {
    ...
    if (nBitsRegistered >= MIN_BITS && nBitsRegistered % 4 == 0) {
        out.isValid = true;
        out.value = value;
...
bool EV1527Protocol::buildPacket(const ProtocolData& data, uint16_t* outTimings, size_t& outCount, size_t maxCount) {
...
void EV1527Protocol::setSyncLength(uint16_t newSyncLength) {
